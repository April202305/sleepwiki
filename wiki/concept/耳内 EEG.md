---
type: concept
aliases: ["ear-EEG", "in-ear EEG", "耳 EEG"]
created: 2026-08-18
updated: 2026-08-18
sources: ["[[source-pages/mikkelsen-2019-ear-eeg-whole-night-sleep]]", "[[source-pages/pazuelo-2024-in-ear-signal-quality]]", "[[source-pages/mohamed-2023-wearable-eeg-review]]"]
review_sections: ["2.1"]
status: active
review_due: 2027-08-18
---

# 耳内 EEG

## 定义

耳内 EEG 指在耳道、耳甲腔或耳周放置电极采集脑电的可穿戴形态。它可降低头皮布置负担，但导联位置、接触、湿气、运动和参考配置均会影响可用性。[[source-pages/mikkelsen-2019-ear-eeg-whole-night-sleep]]（PDF 第 1、3–4 页）

## 在睡眠分期中的作用

Mikkelsen 等在 20 名健康参与者的 80 次整夜居家记录中，同步使用干接触耳内 EEG 与部分 PSG；Pazuelo 等则在 8 名受试者、超过 70 小时 PSG 睡眠记录中比较耳内与头皮的慢波、纺锤波和分期相关性。[[source-pages/mikkelsen-2019-ear-eeg-whole-night-sleep]]（PDF 第 1、3、8 页）；[[source-pages/pazuelo-2024-in-ear-signal-quality]]（PDF 第 9–12 页）

## 方法、参数或判定标准

- 应记录耳内/耳周电极位置、干/湿接触、参考与地、采样率、PSG 同步、数据剔除和质量检查规则。[[source-pages/mohamed-2023-wearable-eeg-review]]（PDF 第 4–5 页）
- 数据质量问题需区分连接丢失、湿气/屏蔽故障、电极机械缺陷和运动/肌电伪影；不同来源的比例或相关性不能直接横比。[[source-pages/mikkelsen-2019-ear-eeg-whole-night-sleep]]（PDF 第 3–4、10–11 页）；[[source-pages/pazuelo-2024-in-ear-signal-quality]]（PDF 第 6–8 页）

## 相关模型与数据集

- 设备：[[device/干接触耳内 EEG 系统]]、[[device/Naox 耳内 EEG 设备]]
- 概念：[[concept/可穿戴 EEG]]、[[concept/PSG 参考标准]]

## 关联来源

- [[source-pages/mikkelsen-2019-ear-eeg-whole-night-sleep]]
- [[source-pages/pazuelo-2024-in-ear-signal-quality]]
- [[source-pages/mohamed-2023-wearable-eeg-review]]

## ⚠️争议与待核实

- ⚠️耳内 EEG 的信号相关、自动评分和舒适性是不同验证层级；均不等同于实时闭环触发精度或临床疗效。

## 来源

- [[source-pages/mikkelsen-2019-ear-eeg-whole-night-sleep]]
- [[source-pages/pazuelo-2024-in-ear-signal-quality]]
- [[source-pages/mohamed-2023-wearable-eeg-review]]
