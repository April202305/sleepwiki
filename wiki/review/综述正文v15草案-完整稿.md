...
从“睡眠监测”向“睡眠调控”的过渡，核心在于将连续状态感知实时转化为干预决策。离线监测主要回答整夜睡眠结构与事件分布，实时识别进一步判断当前睡眠状态；基础闭环或一级触发闭环则以实时检测到的睡眠阶段、脑电事件或振荡相位作为反馈变量，决定是否刺激及何时刺激。更高阶的自适应闭环还可在此基础上评价刺激后的生理响应，并据此更新后续刺激参数。与从随机慢振荡相位启动、随后按预设间隔递送刺激的开放环方案相比，闭环系统强调干预时机与实时神经生理状态的匹配（[[source-pages/weigenand-2016-open-loop-timing]]，PDF 第 1–3 页，Figure 1）。因此，单纯按照预设时程播放刺激属于开放环；仅以一次睡眠阶段判断启动后续固定序列，更适合称为阶段触发或状态依赖刺激；持续读取脑状态并据此控制刺激时机的系统可归入一级闭环，而依据刺激后响应继续更新参数的系统属于更高阶自适应闭环（[[source-pages/esfahani-2023-closed-loop-auditory-principles]]，PDF 第 3、12 页）。
...
![可穿戴睡眠闭环干预系统架构](assets/wearable-sleep-closed-loop-bci-architecture-no-caption.png)
...
本文采用结构化叙述性综述方法，围绕闭环睡眠干预“感知—识别与决策—刺激—反馈—验证”全链路开展文献检索。外文数据库涵盖PubMed、OpenAlex、Scopus、Semantic Scholar、ClinicalTrials.gov，中文数据库包括中国知网、万方、SinoMed及维普，并通过引文追溯、定向文献补充完善检索体系。检索词围绕可穿戴脑电、睡眠分期、闭环刺激、慢波振荡、实时相位估计、系统时延、居家睡眠干预等核心关键词组合设置，覆盖领域核心研究方向。

本研究设置六条检索主线，覆盖闭环架构与时延、慢波锁相刺激、全睡眠周期干预、刺激模态与系统形态、个体化干预与安全性、可穿戴脑电与PSG验证六大研究维度。经线内去重得到文献786条，全局去重后保留647篇，同步纳入注册临床试验223项。因单篇文献可适配多条检索主题线，各主题检索数量不做累加统计。本文最终引用文献109篇，涵盖指南、综述、算法设备研究、人体干预试验、会议论文、预印本、动物实验及相关工程文献，该统计口径不同于系统综述的最终纳入文献数量。
...
所有文献经跨平台去重、题录摘要初筛、全文核验三级筛选流程，系统提取纳入研究的人群特征、设备信号参数、运行模式、刺激参数、时延指标、研究终点、安全性及试验质控等核心信息。本研究严格分层整合证据，区分信号质量、在线识别、刺激命中、生理效应、临床获益与长期安全多级证据，规避以离线性能替代在线表现、以瞬时脑电响应替代临床疗效的论证偏差。因未开展双人独立筛选、标准化偏倚风险评价及完整PRISMA规范化筛选流程，本文定位为结构化叙述性综述，非系统综述或荟萃分析。
...
睡眠电、磁刺激研究可依据控制逻辑分为三类闭环或反馈模式，并将开放环刺激作为对照类型。第一类由实时 EEG 睡眠状态或脑电事件触发刺激；第二类依据内源脑电信号持续更新刺激相位、频率或其他参数；第三类使一种外加刺激锁定另一种外加波形的预设相位。开放环刺激则按照固定波形或时程运行，不使用实时脑电参与决策。上述类型在反馈变量、相位参考和重新触发机制方面存在差异，因此宜按照控制关系分类，而不能仅依据电刺激或磁刺激的硬件模态合并比较。

电、磁刺激研究的比较首先需要区分三个问题：实时 EEG 是否进入刺激决策，相位基准来自内源脑活动还是外加波形，以及刺激期间是否存在无法解释脑电信号的监测盲区。表 5-2 据此从实时 EEG 决策、相位基准、刺激伪影或信号盲区、重新触发条件和研究角色等方面，比较不同电、磁刺激控制关系。表中的开放环刺激用于提供机制或对照参照，不属于严格意义上的闭环控制。
...
| --- | --- | --- | --- | --- | --- |
| EEG状态/事件触发刺激 | 是 | NREM阶段、纺锤波或慢波事件 | 刺激期间EEG可能形成解释盲区 | 暂停期结束后重新检测；规则依研究报告 | Lustenberger等；直接闭环证据 |
| 持续反馈控制tACS | 是 | 内源慢波相位或频率 | 在线伪影抑制和反馈盲区需逐项核验 | 依据恢复后的脑电状态重新判断 | Ketz/Jones；直接反馈控制证据 |
| 外加波形相位同步 | 否 | 外加tACS等预设波形相位 | 存在组合刺激伪影 | 依外加波形和预设协议推进 | Takahashi等；邻近技术证据 |
...
| MHSL-SB 系统族 | NREM 触发、状态/慢波门控 | 声音控制 | 时延仍未充分报告 | 有多夜响应 | 工程与多夜人体配对；纳入夜与完成者分母部分报告 | NREM、beta 与觉醒门控 |
| TSB Axo | 实时 NREM 门控与慢波检测 | 目标相位与个体音量 | 时延仍未充分报告 | 有事件响应和功能终点 | 居家干预夜；失败夜仍需充分报告 | 稳定 NREM、觉醒、ON/OFF 和个体音量 |
| CLAS-hdEEG | N3 实验条件和慢波检测 | 90° 目标窗命中 | 检测至 DIN；声音到耳仍未实测 | 有短窗响应 | 实验室系统 | 阈值/波形门控；觉醒停止仍需充分报告 |
| Henao 耳内 EEG 闭环 | 耳内慢振荡检测 | 两声目标窗命中 | 局部滤波延迟；声音到耳仍未实测 | 有短窗响应 | 实验室午睡 | 双声后暂停；觉醒停止仍需充分报告 |
| 耳内 EEG 监测候选 | PSG 与信号质量验证 | 监测候选 | 采集同步 | 监测证据 | 有居家采集和电极可用性 | 刺激控制待接入 |
...
闭环睡眠干预的证据链分为六大核心层级，涵盖信号质量、在线状态识别、刺激目标窗命中、神经生理调制、行为/临床终点及长期安全效益。当前多数研究仅验证了刺激锁定的慢波、纺锤波、振荡耦合等瞬时生理指标变化，但这类局部、短时的生理响应，无法直接等价于睡眠质量提升、记忆稳定性增强或临床症状改善（[[source-pages/henin-2019-clas-oscillations-not-memory]]，PDF 第 1、6–10 页；[[source-pages/jourde-2026-so-spindle-stimulation]]，PDF 第 1、7–12 页）。

现有研究证实，慢振荡、纺锤波的生理调制与行为改善无稳定同步关系，干预效果显著受任务类型、干预方案、受试样本等因素调控。Jourde 等、Baldassarri 等研究发现，部分受试可观测到明显生理指标变化，但行为获益结果不稳定；Clark 等则在多夜干预方案与复杂学习任务场景中，验证了正向行为改善效果。由此可见，闭环睡眠干预的效果并非全域有效或完全无效，需独立区分刺激命中效果、生理响应特征与行为终点收益分层分析（[[source-pages/jourde-2026-so-spindle-stimulation]]，PDF 第 1、7–12 页；[[source-pages/baldassarri-2026-clas-memory-ecological]]，PDF 第 1、3–9 页；[[source-pages/clark-2024-clas-language-discovery-learning]]，PDF 第 1、3–8 页）。

Harlow 等的元分析研究发现，睡眠干预的记忆效应量随发表年份呈下降趋势，证实盲法管控质量、结局测量可靠性会直接影响研究结果的解读有效性（[[source-pages/harlow-2023-memory-acoustic-stimulation-meta-review]]，PDF 第 1、9–16 页）。此外，tACS 波形、rTMS 相位同步干预研究也出现“生理指标变化、行为获益阴性”的分离现象，但此类技术未依托内源 EEG 实时反馈校准相位基准，仅可作为邻近技术参照，无法与 EEG 闭环研究合并效应量分析（[[source-pages/takahashi-2026-phase-synchronized-rtms-tacs]]，PDF 第 6–10 页）。整体而言，生理响应到行为获益的因果链路断裂、稳定性不足，是当前闭环睡眠干预的核心短板。

### ## 6.2 干预疗效个体差异显著，参数缺乏个体化适配机制

闭环睡眠干预存在极强的个体异质性，贯穿信号采集、生理响应、临床特征、跨夜状态波动全流程。设备电极接触状态、环境噪声会直接影响睡眠信号在线识别精度；年龄、慢波振幅、纺锤峰频、听觉阈值的个体差异，会改变有效刺激窗口与神经响应效果；患者群体的基础疾病、用药情况、共病特征，也会显著干扰干预结果，需针对性控制变量。现有研究与设备应用数据证实，群体平均干预结果无法直接推导个体稳定获益，信号采集质量、年龄相关神经易感性是实现个体化干预的核心前置条件（[[source-pages/mohamed-2023-wearable-eeg-review]]，PDF 第 4–5 页；[[source-pages/mikkelsen-2019-ear-eeg-whole-night-sleep]]，PDF 第 3–4、8 页；[[source-pages/schneider-2020-clas-susceptibility-age]]，PDF 第 1、5–11 页）。

个体化干预优化主要分为模型适配与刺激参数适配两类。模型适配聚焦睡眠分期、慢波与纺锤波事件检测模型的设备适配、个体适配及跨夜迭代更新；刺激参数适配侧重触发阈值、目标相位、刺激强度、干预间隔、累计剂量的个性化校准。现有持续学习、迁移学习研究已验证了识别模型跨设备、跨夜间适配的可行性，但相关成果仅停留在模型验证阶段，尚未实现睡眠过程中识别模型与刺激控制器的同步、稳定、可解释在线迭代更新（[[source-pages/sobral-2025-continual-learning-spindle]]，PDF 第 1、5–12 页；[[source-pages/radke-2024-transfer-learning-sleep-staging]]，PDF 第 1、5–11 页）。

可穿戴多模态分期技术可作为域偏移优化、轻量化睡眠分期的参考方法，但无法替代基于 EEG 信号的事件检测与相位闭环调控。纯加速度睡眠分期无 EEG 靶信号支撑，无法构建精准刺激链路，多模态无 EEG 模型也不能实现相位锁定的闭环干预（[[source-pages/canton-2026-wearable-staging-personalization]]，PDF 第 1–4 页；[[source-pages/zhang-2026-mamba-wearable-staging]]，PDF 第 1、3–13 页）。FlexSleepTransformer 模型具备多通道配置、混合数据训练优势，可支撑干预输入配置的个性化适配优化（[[source-pages/guo-2024-flexsleeptransformer]]，PDF 第 4–12 页）。总体来看，当前技术缺乏成熟的双自适应个体化调控机制，无法匹配不同个体、不同时段的睡眠干预需求。

## 6.3 实时相位估计存在精度与时延双重瓶颈

闭环睡眠相位调控的精度偏差、系统时延问题，由睡眠慢波生理特性、在线因果算法约束、硬件传输链路延迟三重因素共同导致。慢波振幅低、频率非平稳、波形不对称的生理特征，造成瞬时相位难以精准界定；在线因果算法仅能调用实时与历史信号，无法复用离线双向处理的高精度计算逻辑；同时，信号采集缓冲、无线传输、系统调度、音频驱动与硬件响应的全链路延迟，会造成算法判定时点与刺激实际作用时点的时序偏移。因此，仅依靠算法运算耗时、离线相位重建数据，无法验证人体真实干预的目标相位命中效果（[[source-pages/esfahani-2023-closed-loop-auditory-principles]]，PDF 第 3–5、12–13 页；[[source-pages/ferster-2022-real-time-phase-algorithms]]，PDF 第 1–2、7 页）。

多项研究表明，闭环干预效果需结合目标相位、慢波振幅、圆周误差、补偿间隔、端到端时延多维度联合评价。尤其在低幅慢波场景下，系统误触发、漏触发、相位偏移问题高发，且此类异常会直接改变刺激后 EEG 响应效果，甚至提升睡眠觉醒风险，是相位调控的关键短板（[[source-pages/santostasi-2015-pll-acoustic-stimulation]]，HTML Results 3.3–3.4；[[source-pages/ferster-2022-real-time-phase-algorithms]]，PDF 第 1、7 页；[[source-pages/piorecky-2021-real-time-slow-oscillation]]，PDF 第 6、12–20 页）。

相位偏差、滤波延迟、刺激落点时序错配属于软硬件耦合性问题，需依托统一技术框架完成标准化评测。当前行业缺乏统一基准体系，尚无广泛通用的公开基准数据集，可同步覆盖人体原始睡眠 EEG、刺激物理时间戳、全模块时延、异常触发日志等核心数据。Thalamus 系统可提供时钟同步、负载测试、失效保护的工程参考，但无法替代人体睡眠“采集—计算—刺激”全链路的真实场景验证（[[source-pages/haggerty-2026-thalamus]]，PDF 第 8–16 页）。整体而言，精度与时延的双重瓶颈，是制约闭环干预精准性、可复现性的核心技术障碍。

## 6.4 长期干预的安全性与睡眠结构扰动风险

当前闭环睡眠干预的安全证据体系存在明显短板，现有安全性数据多来源于单夜实验室实验、短期居家试点研究，缺乏长期、高频、常态化居家使用的安全随访数据。短期研究记录的无严重不良事件结果，仅可证明短期干预的人体耐受性，无法外推至长期临床应用场景。干预即时风险主要包括微觉醒、完全觉醒、自主神经波动及听觉、躯体机械不适；长期潜在风险则涵盖神经响应习惯化、干预效应衰减、用户依从性下降及未知累积损伤。同时，慢波靶向调制会联动改变纺锤波、涟漪神经活动与睡眠阶段转换规律，因此安全评价需覆盖整夜睡眠结构稳定性与次日日间认知、躯体功能状态（[[source-pages/lustenberger-2022-clas-older-adults-home]]，PDF 第 1、6–12 页；[[source-pages/aksamaz-2023-clas-memory-ripple]]，HTML Results/Discussion）。

现有多夜居家研究仅能初步验证短期干预耐受性，无法解答长期使用、疾病人群、合并用药、高频干预场景下的安全隐患。Aksamaz 等的动物实验仅可提示纺锤波、涟漪活动的潜在机制风险，不能作为人体长期安全的实证依据；tACS、rTMS 相位同步干预的安全数据仅适用于短期暴露场景，无法弥补 EEG 闭环长期干预的安全证据缺口（[[source-pages/lustenberger-2022-clas-older-adults-home]]，PDF 第 1、6–12 页；[[source-pages/aksamaz-2023-clas-memory-ripple]]，HTML Results/Discussion；[[source-pages/takahashi-2026-phase-synchronized-rtms-tacs]]，PDF 第 10–16 页）。目前行业尚未形成标准化的长期安全管控与评价体系，难以支撑常态化居家闭环干预的安全落地。

## 6.5 居家真实世界证据与标准化评测缺失

闭环睡眠干预的真实世界应用需区分实验室效能、居家应用效果与临床落地实施三大维度：实验室效能验证受控条件下的靶向神经响应能力，居家效果评估长期多夜干预的持续获益，落地实施则聚焦设备佩戴、用户依从性、数据完整性、临床适配性。当前各类可穿戴设备研究的 PSG 配置、受试人群、睡眠评分标准、数据筛选规则差异极大，导致不同研究的准确率数据无法横向对比，易混淆设备性能、算法偏差与临床干预效果（[[source-pages/de-gans-2024-eeg-wearables-systematic-review]]，PDF 第 1、9–10 页；[[source-pages/markov-2025-wearable-eeg-meta-analysis]]，PDF 第 1、5–12 页）。

可穿戴 EEG 居家验证研究证实，单一整体准确率指标无法客观反映真实场景应用效果。睡眠分期逐期偏差、人工评分误差、电极配置差异、夜间信号质量波动、失败夜剔除规则等，都会改变最终评测结论；N1 期、REM 期及睡眠阶段转换节点的误差，极易被整夜平均数据掩盖，造成效果评估偏差（[[source-pages/kjaer-2022-ear-eeg-repeated-scoring]]，PDF 第 1、6–11 页；[[source-pages/markov-2025-wearable-eeg-meta-analysis]]，PDF 第 1、5–12 页）。现有研究普遍仅统计有效干预夜的平均性能，忽略了信号采集失败、数据无法评分、刺激触发失效、用户中途退出等无效样本，导致真实世界证据存在严重偏倚。

目前行业缺乏统一的最小报告规范与外部验证框架，设备参数、算法版本、时延指标、数据筛选、安全评价等核心信息披露不完整，亚组分析、低质量场景评测、长期随访数据缺失，无法形成标准化、可对比、可复现的真实世界证据体系，严重制约了技术的临床落地与迭代优化。

## 7 结论与优先研究问题

## 7.1 主要结论

本文围绕“感知—识别决策—刺激输出—反馈调节—效果验证”的完整技术链路，系统梳理了可穿戴闭环睡眠干预的技术基础、应用场景、刺激模态与实证研究证据。现有技术已实现低通道可穿戴 EEG 的睡眠分期门控、慢波与纺锤波事件检测、相位依赖式靶向刺激，为居家闭环睡眠干预奠定了基础技术条件。但当前研究仍存在明显短板，信号长期可用性、在线因果运算可靠性、刺激物理落点精准度、无效数据处理规则均缺乏统一标准，离线算法的高精度性能无法等同于真实闭环系统的实际刺激命中能力（[[source-pages/de-gans-2024-eeg-wearables-systematic-review]]，PDF 第1、9–10页；[[source-pages/debellemaniere-2018-ambulatory-dry-eeg-closed-loop]]，PDF 第1、4–9页；[[source-pages/ferster-2022-real-time-phase-algorithms]]，PDF 第1–2、7页；[[source-pages/valenchon-2022-portiloop]]，PDF 第4、6、13–15页）。

干预效果层面，闭环听觉刺激可在特定实验条件下诱发慢波、纺锤波、振荡耦合的短时生理改变，但该类瞬时生理响应无法稳定转化为整夜睡眠质量改善、认知行为提升或临床症状缓解。少数多夜干预研究观测到特定任务、特定人群的正向获益，但干预效果高度依赖刺激范式、实验任务、受试人群、干预剂量与测量精度，整体证据异质性极高，尚未形成统一的认知增强、疾病治疗有效性结论。同时，电、磁、振动、光、温度等不同干预模态的反馈逻辑、设备惯性、伪影干扰、安全风险差异显著，无法脱离专属控制机制与验证体系进行横向对比（[[source-pages/jourde-2026-so-spindle-stimulation]]，PDF 第1、7–12页；[[source-pages/harlow-2023-memory-acoustic-stimulation-meta-review]]，PDF 第1、9–16页；[[source-pages/clark-2024-clas-language-discovery-learning]]，PDF 第1、3–8页；[[source-pages/baldassarri-2026-clas-memory-ecological]]，PDF 第1、3–9页）。

综合来看，可穿戴闭环睡眠干预目前处于**技术可行、临床证据待积累**的发展阶段，属于极具潜力的候选干预技术。技术迭代不能单纯追求离线算法精度提升或短时脑电响应强化，需构建涵盖信号可靠性、在线因果决策、刺激精准命中、生理效应可复现、临床获益稳定、长期安全可控、真实场景可落地的完整证据链（[[source-pages/esfahani-2023-closed-loop-auditory-principles]]，PDF 第3、12–13页；[[source-pages/dudysova-2024-closed-loop-insomnia-pilot]]，PDF 第1页；[[source-pages/lustenberger-2022-clas-older-adults-home]]，PDF 第1、6–12页）。
## 7.2 优先研究问题及决策意义

针对第6章节梳理的核心技术与实证短板，依据证据链完善程度，将未来研究工作划分为四个递进式优先层级，逐级完善闭环睡眠干预的技术体系与临床证据体系。

第一优先级是构建可核验的测量体系与安全基础。核心研究目标为明确真实居家场景下的系统运行有效性与基础安全性，重点厘清四类核心指标：居家监测夜的有效记录率、信号合格夜占比、成功刺激夜占比；合格干预机会下的信号检测、指令触发、刺激执行、目标窗命中效率；信号采集、数据预处理、算法推理、通信传输、物理刺激全链路的分项时延；低质量信号、安全拦截、误触发、执行失败的核心诱因。该层级研究是闭环人体试验、跨设备对比、自动化干预落地的基础前提，可规避设备故障、算法偏差、无效数据剔除带来的疗效评价偏倚（[[source-pages/de-gans-2024-eeg-wearables-systematic-review]]，PDF 第9–10页；[[source-pages/bressler-2023-wearable-eeg-closed-loop]]，PDF 第4–5、13–14页；[[source-pages/valenchon-2022-portiloop]]，PDF 第13–15页；[[source-pages/bazregarzadeh-2026-clas-hdeeg]]，Methods 2.1–2.4、Results 3）。

第二优先级是验证生理响应与临床获益的因果链路。基于基础测量体系，破解“生理调制—行为获益”链路不稳的核心问题，重点验证：刺激相位与事件的真实命中有效性；命中质量对慢波、纺锤波及其耦合活动的调控作用；生理改变对睡眠质量、记忆功能、情绪状态、临床症状的中介改善效应；刺激剂量、干预间隔与干预效果的量效关系。该层级研究决定技术定位，区分其是神经生理研究工具，还是具备临床应用潜力的干预手段。研究需采用假刺激盲法、预注册核心终点，通过分层分析、中介分析明确因果关联（[[source-pages/jourde-2026-so-spindle-stimulation]]，PDF 第1、7–12页；[[source-pages/harlow-2023-memory-acoustic-stimulation-meta-review]]，PDF 第1、9–16页；[[source-pages/clark-2024-clas-language-discovery-learning]]，PDF 第1、3–8页；[[source-pages/baldassarri-2026-clas-memory-ecological]]，PDF 第1、3–9页）。

第三优先级是建立可解释的个体化适配调控机制。针对干预个体异质性问题，构建精准自适应干预体系，核心研究内容包括：筛选可稳定预测干预响应的核心个体指标（年龄、疾病、用药、听觉阈值、脑电特征等）；量化模型适配、刺激参数适配对干预效果的增益贡献；验证识别模型与刺激控制器联合在线更新的安全性、有效性，规避误触发、剂量超标风险。该层级成果可明确个体化干预方案（固定参数、单次校准、持续自适应），为临床试验人群分层、个体剂量优化、响应者筛选提供依据（[[source-pages/schneider-2020-clas-susceptibility-age]]，PDF 第1、5–11页；[[source-pages/sobral-2025-continual-learning-spindle]]，PDF 第1、5–12页；[[source-pages/radke-2024-transfer-learning-sleep-staging]]，PDF 第1、5–11页）。

第四优先级是开展疾病人群长期真实世界临床验证。在前三级技术与实证基础上，推进临床落地验证，重点研究长期多夜使用场景下的系统稳定性、用户依从性、神经响应适应性、睡眠结构安全性，以及失眠、轻度认知损害、阿尔茨海默病等患病人群的临床获益。通过多中心随机对照试验、全样本数据分析（包含失败夜）、长期随访，综合权衡临床获益与设备负担、觉醒风险、累积安全风险，判定技术的临床推广价值（[[source-pages/dudysova-2024-closed-loop-insomnia-pilot]]，PDF 第1页；[[source-pages/papalambros-2019-acoustic-mci]]，PDF 第1、4–8页；[[source-pages/vandenbulcke-2023-acoustic-ad-jcsm]]，PDF 第1、3–8页；[[source-pages/zeller-2024-multi-night-acoustic-cognitive-impairment]]，PDF 第1、4–12页；[[source-pages/lustenberger-2022-clas-older-adults-home]]，PDF 第1、6–12页）。

贯穿四级研究的核心共性任务，是搭建标准化、可共享的数据集与报告体系，整合原始可穿戴信号、PSG 参考标签、刺激时间戳、全链路时延、干预后响应、无效数据、安全事件等核心数据，形成可审查、可对比、可复现的完整证据链（[[source-pages/kothe-2025-lsl-sync]]，§1.2、§2.2、§3；[[source-pages/haggerty-2026-thalamus]]，PDF 第8–16页）。
