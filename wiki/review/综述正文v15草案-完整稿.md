...
本研究采用结构化叙述性综述研究范式，围绕闭环睡眠干预“信号感知—识别与决策—刺激输出—效果反馈—效果验证”的完整技术链路，开展系统性中英文文献检索与筛选工作。文献检索工作于2026年8月13日至15日集中完成，外文检索数据库涵盖PubMed、OpenAlex、Scopus、Semantic Scholar 及ClinicalTrials.gov；中文检索数据库选取国内主流学术数据库，包括中国知网、万方数据库、中国生物医学文献数据库（SinoMed）及维普中文科技期刊数据库。同时通过纳入文献溯源、核心综述引文追溯、研究主题文献清单定向补充等方式，完善文献检索体系。检索主题词围绕睡眠监测、可穿戴脑电、睡眠分期、闭环刺激、慢波振荡、睡眠纺锤波、相位估计、实时调控、系统时延、居家睡眠干预等核心维度组合设定，全面覆盖本研究中英文研究范畴。

本次检索围绕研究核心设置六条专项检索主题主线，分别聚焦闭环干预架构与系统时延特性、慢波锁相听觉刺激干预机制、入睡期与快速眼动期等全睡眠周期干预方案、多类型刺激模态与系统实现形态、个体化干预策略与长期应用安全性、可穿戴脑电检测与多导睡眠监测的有效性验证六大研究方向。经各主题线内部去重后，共得到786条文献记录，对多主题线交叉重复文献进行全局去重后，最终保留有效文献记录647篇，同步检索筛选得到相关注册临床试验223项。由于单篇文献可同时匹配多条检索主题线，各主题线检索数量不具备直接累加统计的意义。本综述正式定稿共引用文献107篇，文献类型涵盖行业指南、学术综述、算法与设备技术研究、人体睡眠干预试验、会议论文、预印本成果、动物实验研究及相关工程技术文献，该文献统计口径区别于系统综述的最终纳入文献数量，仅为本研究论述支撑文献总量。

本研究的文献纳入标准以支撑闭环睡眠干预全链路研究为核心，凡可直接佐证睡眠信号采集质量、在线睡眠状态与事件识别精度、脑电相位估计效能、刺激触发控制机制、系统运行时延特征、睡眠生理响应规律、行为与临床干预结局、干预安全性及居家场景落地实施等任一核心环节的中英文文献，均予以纳入。相应的文献排除标准为：未设置睡眠相关研究终点、仅汇报离线实验结果且无法支撑闭环干预完整链路、仅含产品宣传类内容、研究方法与题录信息无法核验的文献。为保障研究内容的前沿性与全面性，本研究对不同类型文献的应用边界进行严格界定：预印本成果用于补充领域最新探索性研究进展；会议论文主要支撑相关算法设计与工程实现层面的论述；动物实验研究仅用于阐释睡眠干预底层机制与潜在应用风险提示；非睡眠领域工程研究仅作为系统同步、参数标定与失效保护的参照依据，上述各类文献均不直接用于推导人体睡眠干预的临床疗效结论。

所有检索获取的中英文文献均经过跨平台重复剔除、题名与摘要初步筛选、全文深度核验三级筛选流程。对最终纳入的文献进行标准化信息提取，核心提取内容包括研究人群特征、信号采集设备与脑电信号类型、睡眠检测参考标准、系统运行模式、算法因果有效性、刺激参数设置、系统分项时延指标、生理与行为学研究终点、干预安全性评价及失效睡眠夜的处理方案等关键研究数据。

在文献证据整合与分析阶段，本研究严格划分证据层级，从信号采集质量、在线状态识别能力、刺激精准命中效果、睡眠生理响应效应、行为与临床干预获益、长期应用安全性六个维度开展分层论证，规避学术论证偏差，杜绝以离线算法准确率替代系统在线运行性能、以瞬时脑电响应效果等同于长期临床干预疗效的片面论证方式。鉴于本研究未开展双人独立文献筛选、标准化偏倚风险评价及完整PRISMA规范筛选流程，本文研究定位为结构化叙述性综述，不界定为系统综述或荟萃分析。
...
非听觉模态不宜仅依据生理效应大小进行排序，还需要联合考察反馈变量、睡眠状态触发、执行惯性、机械或光学伪影、佩戴负担和觉醒保护。尤其是振动和温度具有明显的机械或热惯性，其停止指令与生理作用终止之间可能存在时间差，需要与声音或电刺激采用不同的时延和安全评价口径。
...
此外，无 EEG 多模态睡眠分期模型、非睡眠场景的 Thalamus 同步平台、开放环温控床罩、外加 tACS 与 rTMS 相位同步方案，以及通过醒后报告形成反馈的 Dormio，可分别为多模态分期、统一时钟和失效保护、环境控制、多刺激同步及梦境交互提供局部技术参照（[[source-pages/zhang-2026-mamba-wearable-staging]]；[[source-pages/haggerty-2026-thalamus]]；[[source-pages/stevenson-2025-temperature-mattress]]；[[source-pages/takahashi-2026-phase-synchronized-rtms-tacs]]；[[source-pages/haar-horowitz-2020-dormio]]）。这些研究不能直接视为完整睡眠闭环干预系统；引用时应明确其支撑的具体环节、运行条件和尚未覆盖的睡眠内反馈链路。

## 6 关键挑战与展望

### 6.1 生理增强与行为学获益的因果链条尚不稳固

闭环睡眠干预的证据链可分为信号质量、在线状态识别、刺激精准命中、神经生理调制、行为或临床终点以及长期安全效益六个层级。本文纳入研究中相当一部分主要报告刺激锁定的慢波、纺锤波或振荡耦合变化，而这些瞬时或局部生理响应不能直接等同于睡眠质量改善、记忆稳定性提高或临床症状缓解（[[source-pages/henin-2019-clas-oscillations-not-memory]]，PDF 第 1、6–10 页；[[source-pages/jourde-2026-so-spindle-stimulation]]，PDF 第 1、7–12 页）。
...
Harlow 等的元分析显示，记忆效应量随发表年份呈下降关联，并指出盲法维持和结局测量可靠性会影响结果解释（[[source-pages/harlow-2023-memory-acoustic-stimulation-meta-review]]，PDF 第 1、9–16 页）。外加 tACS 波形与 rTMS 相位同步研究也可呈现生理变化与行为阴性的分离，但其相位基准并非内源 EEG 实时反馈，只能作为邻近技术参照，不能与 EEG 闭环研究合并效应量（[[source-pages/takahashi-2026-phase-synchronized-rtms-tacs]]，PDF 第 6–10 页）。未来研究应预注册主要终点，设置能够维持盲法的假刺激及相位、剂量对照，并通过分层分析和中介分析检验刺激靶点、生理响应与行为变化之间的路径；个体内多夜模型还可用于考察生理响应者与行为响应者是否一致。证据包：[[review/证据包/06-挑战展望-6.1-P1-证据包]]。
...
现有闭环慢波干预的安全证据主要来自单夜实验室研究或短期居家试验，未发生严重不良事件只能支持相应暴露范围内的短期耐受性，不能直接外推至长期居家使用。即时风险包括微觉醒、完全觉醒、自主神经波动以及听觉、皮肤或机械不适；长期研究还需监测神经响应习惯化、效应衰减、依从性变化和未知累积风险。慢波调制还可能伴随纺锤波、涟漪活动或睡眠阶段转换的变化，因此安全评价应覆盖整夜睡眠结构和次日日间功能（[[source-pages/lustenberger-2022-clas-older-adults-home]]，PDF 第 1、6–12 页；[[source-pages/aksamaz-2023-clas-memory-ripple]]，HTML Results/Discussion）。

多夜居家研究可提供初步耐受性线索，但不足以回答数月使用、疾病人群、合并用药或高频使用的安全问题；Aksamaz 等的动物研究仅用于提示可能涉及纺锤波和涟漪活动的机制风险，不能作为人体长期安全证据。外加 tACS 波形与 rTMS 相位同步研究也只能作为短期暴露的邻近参照，不能补齐 EEG 反馈闭环的长期安全证据（[[source-pages/lustenberger-2022-clas-older-adults-home]]，PDF 第 1、6–12 页；[[source-pages/aksamaz-2023-clas-memory-ripple]]，HTML Results/Discussion；[[source-pages/takahashi-2026-phase-synchronized-rtms-tacs]]，PDF 第 10–16 页）。未来协议应预设每夜剂量上限、连续觉醒停机、设备故障保护、次日嗜睡与认知监测以及停用后随访。安全报告除不良事件外，还应提供暴露分母，例如每 1000 次刺激的微觉醒或觉醒次数、每人夜不良事件与设备不适、累计刺激剂量及触发停机规则的夜数，并按刺激模态和人群分层报告，使安全评价由事后记录转向主动风险管理。证据包：[[review/证据包/06-挑战展望-6.4-P1-证据包]]。
...
最小报告集应包括设备与电极配置、固件和模型版本、运行环境及代码或可复现实现信息，PSG 同步与时钟方案，因果运行方式和分项时延，训练、验证、测试及外部验证的数据划分，有效数据比例、缺失夜定义、失败夜处理和敏感性分析，以及逐期指标、置信区间、效应量、评分者一致性、概率校准、最差亚组表现、刺激命中、觉醒、剂量、患者报告结局、不良事件和隐私措施。外部验证应覆盖不同中心、年龄、疾病人群和设备，并报告亚组与校准漂移及低质量夜表现；临床试验宜采用多夜随机对照、能够有效维持的盲法、预注册主要终点和充分随访。未来标准化工作应建立可共享的时间戳数据资源、因果算法基准、失败夜与退出分母、分层安全报告及外部验证框架，从而推动设备性能、居家实施效果和临床获益形成彼此衔接但不相互替代的证据体系。证据包：[[review/证据包/06-挑战展望-6.5-P1-证据包]]。

## 7 结论与优先研究问题

### 7.1 主要结论

本文围绕“感知—识别与决策—刺激—反馈—验证”链路，对可穿戴闭环睡眠干预的技术基础、应用场景、刺激模态和验证证据进行了综合分析。现有研究表明，可穿戴及低通道脑电已经能够支持部分睡眠阶段门控、慢波和纺锤波事件检测及相位依赖刺激，为居家闭环干预提供了初步技术基础。然而，信号可用性、在线因果运行、刺激物理到达时间和失败夜处理仍缺少统一报告，离线算法性能不能直接代表真实闭环系统的刺激命中能力（[[source-pages/de-gans-2024-eeg-wearables-systematic-review]]，PDF 第1、9–10页；[[source-pages/debellemaniere-2018-ambulatory-dry-eeg-closed-loop]]，PDF 第1、4–9页；[[source-pages/ferster-2022-real-time-phase-algorithms]]，PDF 第1–2、7页；[[source-pages/valenchon-2022-portiloop]]，PDF 第4、6、13–15页）。
