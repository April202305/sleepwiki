---
type: source
aliases: ["Takahashi 2026 phase-synchronized rTMS tACS"]
created: 2026-08-19
updated: 2026-08-22
sources: []
review_sections: ["5.2", "6.1", "6.4"]
status: active
review_due: 2027-08-19
---

# 睡眠阶段特异的相位同步 rTMS 与 tACS

## 基本信息
- 类型：论文
- 原始文件：`raw/inbox/takahashi-2026-phase-sync-rtms-tacs-sleep.pdf`
- source_id：`raw/inbox/takahashi-2026-phase-sync-rtms-tacs-sleep.pdf`
- 作者/机构：Kuri Takahashi、Min-Fang Kuo、Michael A. Nitsche
- 年份：2026
- 英文原题：Sleep stage-specific effects of 0.75 Hz phase-synchronized rTMS and tACS on delta frequency activity during sleep
- 录入日期：2026-08-19
- review_sections：["5.2", "6.1", "6.4"]

## 题录与引用字段
- 题录状态：已核验
- 核验来源：Crossref（DOI 10.1101/2025.05.09.653171），核验日 2026-08-22
- 文献语种：英文
- 作者（原始顺序）：Kuri Takahashi; Min-Fang Kuo; Michael A. Nitsche
- 原始题名：Sleep stage-specific effects of 0.75 Hz phase-synchronized rTMS and tACS on delta frequency activity during sleep
- 文献类型标识：[A/OL]
- 载体或容器题名：bioRxiv
- 出版年：2025
- 卷：
- 期：
- 起止页码：
- 文章号：
- 出版地：
- 出版者：
- 编辑：
- 会议名称：
- 会议地点：
- 会议日期：
- DOI：10.1101/2025.05.09.653171
- URL：https://doi.org/10.1101/2025.05.09.653171
- 发表或更新日期：2025-05-11
- 引用日期：2026-08-22
- 补充标识：
- 核验备注：
- Word 成稿引用：TAKAHASHI K, KUO M, NITSCHE M A. Sleep stage-specific effects of 0.75 Hz phase-synchronized rTMS and tACS on delta frequency activity during sleep[A/OL]. bioRxiv, 2025[2026-08-22]. https://doi.org/10.1101/2025.05.09.653171. DOI:10.1101/2025.05.09.653171.

## 核心摘要

16 名健康年轻成人在睡眠剥夺后完成真实/假刺激被试内平衡设计；0.75 Hz tACS 作为相位基准，rTMS 在 tACS 负峰递送，研究比较不同睡眠阶段的 delta 活动与配对联想记忆。[[source-pages/takahashi-2026-phase-synchronized-rtms-tacs]]（PDF 第 1、10–16 页）

## 方法与发现

- 双侧 TMS 线圈以示波器校准为零相对延迟，真实刺激在 tACS 负峰递送；sham 使用假线圈和短暂 tACS ramp。（PDF 第 12–14 页）
- 研究报告 N2/N3 的阶段特异 delta 变化，但睡眠结构、入睡潜伏期、睡眠效率及配对联想记忆未见显著改善。（PDF 第 6–10 页）
- 刺激相位由外加 tACS 预设，并未根据实时 EEG 慢波检测动态调整；因此属于睡眠阶段内的相位同步组合刺激，不是 EEG 反馈闭环 TMS。（PDF 第 12–16 页）

## 关联词条
- 干预：[[intervention/睡眠相位同步rTMS-tACS]]
- 概念：[[concept/实时相位估计]]

## 局限与待核实
- ⚠️ 小样本、睡眠剥夺和约 3 小时睡眠限制外推；不能解除真正睡眠 EEG 闭环 TMS 缺口。

## 来源
- `raw/inbox/takahashi-2026-phase-sync-rtms-tacs-sleep.pdf`
