---
type: source
aliases: ["Niu 2025", "China OSA prevalence meta-analysis"]
created: 2026-08-17
updated: 2026-08-18
sources: ["raw/inbox/niu-2025-china-osa-prevalence.pdf"]
review_sections: ["1.1"]
status: active
review_due: 2027-08-17
---

# Niu 等（2025）：中国 OSA 患病率的多层次 Meta 分析

## 基本信息

- 类型：系统综述与 Meta 分析
- 原始文件：`raw/inbox/niu-2025-china-osa-prevalence.pdf`
- source_id：`raw/inbox/niu-2025-china-osa-prevalence.pdf`
- 作者/机构：Yuqi Niu 等
- 年份：2025
- 英文原题：Spatiotemporal Trends in the Prevalence of Obstructive Sleep Apnoea Across China: A Multilevel Meta-Analysis Incorporating Geographic and Demographic Stratification (2000–2024)
- DOI：10.2147/NSS.S525547
- review_sections：["1.1"]

## 核心摘要

研究纳入 62 项研究（178,049 人），使用 PSG 或经验证便携监测，并以 2012 AASM AHI≥5 标准诊断；中国汇总[[concept/阻塞性睡眠呼吸暂停]]患病率为 11.8%（95% CI 10.1%–13.4%）。 （PDF 第 1 页）

## 方法与发现

- 患病率从 2000–2005 年的 8.1% 到 2021–2024 年的 26.9%，但抽样策略、地区、性别和研究质量可解释异质性。 （PDF 第 1 页）
- 低质量研究估计更高，二阶段抽样也高于单风险组设计。 （PDF 第 1 页）

## 关联词条

- 概念：[[concept/阻塞性睡眠呼吸暂停]]、[[concept/睡眠障碍疾病负担]]
- 综述：[[review/chapters/01-引言]]、[[review/01-引言-1.1-P1-证据包]]、[[review/证据矩阵]]

## ⚠️局限与待核实

- ⚠️该汇总受 AHI 阈值、抽样策略、研究质量和地区异质性影响；不与失眠或欧洲 OSA 估计合并。

## 来源

- `raw/inbox/niu-2025-china-osa-prevalence.pdf`
